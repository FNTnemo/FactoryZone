...
from map import load_map, map1, map_cells, build_cells, selected_cells
...
def rendering(screen):
    ...
    screen.blit(debug_font.render(f"fps: {fps}, items: {len(items)}, map_cells: {len(map_cells)}, building_cells: {len(build_cells)}, draw_queue: {len(draw_queue[0])}", True, black), (10, 10))
